...
class InvitationRepository {
  ...
  async findByToken(token) {
    const db = await getDatabase();
    return db.get(`SELECT * FROM invitations WHERE token = ?`, [token]);
  }
  ...
  async incrementUsage(id) {
    const db = await getDatabase();
    db.run(`UPDATE invitations SET uses_count = uses_count + 1 WHERE id = ?`, [id]);
    return this.findById(id);
  }

  async revoke(id) {
    const db = await getDatabase();
    db.run(`UPDATE invitations SET is_revoked = 1 WHERE id = ?`, [id]);
    return this.findById(id);
  }
}
